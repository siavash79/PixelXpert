		...
		implements PreferenceFragmentCompat.OnPreferenceStartFragmentCallback
{
	...
	protected void onCreate(Bundle savedInstanceState)
	{
		...
		try {
			Shell.setDefaultBuilder(Shell.Builder.create()
			                                     .setFlags(Shell.FLAG_MOUNT_MASTER)); //access full filesystem
		...
		if(savedInstanceState == null) {
			getSupportFragmentManager().beginTransaction()
			                           .replace(R.id.settings, new HeaderFragment()).commit();
		} else {
		...
		new AlertDialog.Builder(this).setTitle(R.string.nestat_caution_title)
		                             ...
		                             .setPositiveButton(R.string.netstat_caution_yes, (dialogInterface, i) -> {
			                             try {
				                             ...
				                             prefs.edit().putBoolean("NetworkStatsEnabled", false)
				                                  .commit();
				                             Shell.cmd("rm -rf /data/user_de/0/com.android.systemui/netStats")
				                                  .exec();
				                             Thread.sleep(100);
				
				                             prefs.edit().putBoolean("NetworkStatsEnabled", true)
				                                  .commit();
				                             Thread.sleep(100);
				                             prefs.edit()
				                                  .putBoolean("NetworkStatsEnabled", currentStatus)
				                                  .apply();
			                             } catch(Exception ignored) {
			                             }
		                             })
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data)
	{
		...
		if(data == null)
			return; //user hit cancel. Nothing to do
	...
	public static class ThemingFragment extends PreferenceFragmentCompat
	{
		...
		private void updateFontPrefs(SharedPreferences sharedPreferences)
		{
			try {
				...
				boolean FontsOverlayExEnabled = ! sharedPreferences.getString("FontsOverlayEx", "None")
				                                                   .equals("None");
	...
	public static class networkFragment extends PreferenceFragmentCompat
	{
		...
		{
			try {
				findPreference("networkTrafficRXTop").setVisible(prefs.getString("networkTrafficMode", "0")
				                                                      .equals("0"));
				findPreference("networkTrafficColorful").setVisible(! prefs.getString("networkTrafficMode", "0")
				                                                           .equals("3"));
	...
	public static class QuicksettingsFragment extends PreferenceFragmentCompat
	{
		...
		private void updateVisibililty(SharedPreferences sharedPreferences)
		{
			try {
				...
				int displayWidth = getActivity().getWindowManager().getCurrentWindowMetrics()
				                                .getBounds().width();
		...
		private void createPullDownIndicator()
		{
			...
			((ViewGroup) getActivity().getWindow().getDecorView()
			                          .getRootView()).addView(pullDownIndicator);
		}
	}
	...
	public static class GestureNavFragment extends PreferenceFragmentCompat
	{
		...
		private void updateVisibility(SharedPreferences sharedPreferences)
		{
			try {
				...
				int displayHeight = getActivity().getWindowManager().getCurrentWindowMetrics()
				                                 .getBounds().height();
		...
		{
			if((v.getVisibility() == View.VISIBLE) == visible)
				return;
			...
			v.animate().setDuration(duration).alpha(destAlpha)
