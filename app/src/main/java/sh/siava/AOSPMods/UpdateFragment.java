...
public class UpdateFragment extends Fragment
{
		...
		public void onReceive(Context context, Intent intent)
		{
			try {
				if(Objects.equals(intent.getAction(), DownloadManager.ACTION_DOWNLOAD_COMPLETE) && intent.getLongExtra(DownloadManager.EXTRA_DOWNLOAD_ID, - 1) == downloadID) {
					Cursor downloadData = downloadManager.query(new DownloadManager.Query().setFilterById(downloadID));
		...
		binding.radioGroup.setOnCheckedChangeListener((radioGroup, i) -> {
			...
			checkUpdates(result -> {
				...
				getActivity().runOnUiThread(() -> {
					((TextView) view.findViewById(R.id.latestVersionValueID)).setText(String.format("%s (%s)", result.get("version"), result.get("versionCode")));
					int latestCode = 0;
	...
	private void getCurrentVersion()
	{
		rebootPending = false;
		try {
			List<String> updateLines = Shell.cmd("cat /data/adb/modules_update/AOSPMods/module.prop | grep version")
			                                .exec().getOut();
	...
	public void startDownload(String zipURL, int versionNumber)
	{
		...
		downloadID = downloadManager.enqueue(new DownloadManager.Request(Uri.parse(zipURL)).setTitle("AOSPMods Update Package")
	...
	public void notifyInstall()
	{
		...
		//noinspection ConstantConditions
		NotificationCompat.Builder builder = new NotificationCompat.Builder(getContext(), "updates").setSmallIcon(R.drawable.ic_notification_foreground)
