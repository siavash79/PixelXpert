...
public class FlexStatusIconContainer extends LinearLayout
{
	...
	private void initDimens()
	{
		...
		mIconDotFrameWidth = res.getDimensionPixelSize(res.getIdentifier("status_bar_icon_size", "dimen", "android"));
		mIconSize = mIconDotFrameWidth;
		int mDotPadding = getResources().getDimensionPixelSize(res.getIdentifier("overflow_icon_dot_padding", "dimen", getContext().getPackageName()));
		mIconSpacing = getResources().getDimensionPixelSize(res.getIdentifier("status_bar_system_icon_spacing", "dimen", getContext().getPackageName()));
		int radius = getResources().getDimensionPixelSize(res.getIdentifier("overflow_dot_radius", "dimen", getContext().getPackageName()));
	...
	protected void onDraw(Canvas canvas)
	{
		...
		if(DEBUG_OVERFLOW) {
			...
			canvas.drawRect(mUnderflowStart, 0, mUnderflowStart + mUnderflowWidth, getHeight(), paint);
		}
	}
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec)
	{
		try {
			...
			for(int i = getChildCount() - 1; i >= 0; i--) {
				...
				if((boolean) callMethod(icon, "isIconVisible") && ! isBlocked && ! mIgnoredSlots.contains((String) callMethod(icon, "getSlot"))) {
					if(mDotIcon != null) {
					...
					mMaxWidths.add(mMeasureViews.size() == 0 ? iconWidth : Math.max(mMaxWidths.get(mMeasureViews.size() - 1), iconWidth));
					
					mMaxHeights.add(mMeasureViews.size() == 0 ? iconHeight : Math.max(mMaxHeights.get(mMeasureViews.size() - 1), iconHeight));
	...
	public void onViewAdded(View child)
	{
		...
		try {
			...
			child.setTag(getResources().getIdentifier("status_bar_view_state_tag", "id", getContext().getPackageName()), vs);
		} catch(Throwable ignored) {
		}
	}
	...
	public void onViewRemoved(View child)
	{
		...
		child.setTag(getResources().getIdentifier("status_bar_view_state_tag", "id", getContext().getPackageName()), null);
	}
		...
		try {
			...
			for(int i = 0; i < iconCount; i++) {
				...
				if(mTotalRows > 1) {
					...
					float iconTranslationX = sortPlan == SORT_CLEAN ? translationX + ((iconFixedWidth - iconWidth) / 2f) : translationX;
	...
	{
		return child.getTag(getResources().getIdentifier("status_bar_view_state_tag", "id", getContext().getPackageName()));
	}
