...
public class Helpers
{
	...
	public static void getActiveOverlays()
	{
		...
		List<String> lines = com.topjohnwu.superuser.Shell.cmd("cmd overlay list --user 0").exec()
		                                                  .getOut();
	...
	public static void setOverlay(String Key, boolean enabled, boolean force)
	{
		...
		if(activeOverlays == null)
			getActiveOverlays(); //make sure we have a list in hand
...
/*        if (enabled && exclusive) {
		...
		try {
			com.topjohnwu.superuser.Shell.cmd("cmd overlay " + mode + " --user 0 " + packname)
			                             .exec();
		} catch(Throwable t) {
			t.printStackTrace();
		}
	...
	public static SpannableStringBuilder getHumanizedBytes(long bytes, float unitSizeFactor, String unitSeparator, String indicatorSymbol, @Nullable @ColorInt Integer textColor)
	{
		...
		return new SpannableStringBuilder().append(spanSizeString).append(unitSeparator)
		                                   .append(spanUnitString);
	}
	...
	public static boolean installDoubleZip(String DoubleZipped) //installs the zip magisk module. even if it's zipped inside another zip
	{
		try {
			...
			if(unzipper.stream().count() == 1) {
				...
				FileUtils.copy(unzipper.getInputStream(unzipper.entries()
				                                               .nextElement()), unzipOutputStream);
			...
			//install
			Shell.cmd(String.format("magisk --install-module %s", unzippedFile.getAbsolutePath()))
			     .exec();
