...
public class NetworkStats
{
	...
	{
		todayCellRxBytes = todayCellTxBytes = todayRxBytes = todayTxBytes = rxData = txData = 0;
	...
	private void scheduleDateChange()
	{
		try {
			...
			SystemUtils.AlarmManager().set(AlarmManager.RTC, nextDay.getTimeInMillis(), "", () -> {
				resetStats();
				scheduleDateChange();
			}, null);
		} catch(Throwable t) {
	...
	private void saveTrafficData()
	{
		...
		try {
			if(Calendar.getInstance()
			           .get(Calendar.DATE) != operationDate.get(Calendar.DATE)) //in a rare case that we didn't understand the date has changed, this will ensure
			{
				resetStats();
				return;
			}
			...
			jasonWriter.beginObject()
			           ...
			           .name("RXTotal").value(todayRxBytes)
			
			           .name("TXTotal").value(todayTxBytes)
			
			           .name("CellRX").value(todayCellRxBytes)
			
			           .name("CellTX").value(todayCellTxBytes)
	...
	{
		@SuppressLint("SimpleDateFormat") SimpleDateFormat nameFormat = new SimpleDateFormat("yyyyMMdd");
		String filename = String.format("TrafficStats-%s.txt", nameFormat.format(Calendar.getInstance()
		                                                                                 .getTime()));
