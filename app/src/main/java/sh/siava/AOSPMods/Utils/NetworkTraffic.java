...
public class NetworkTraffic extends LinearLayout
{
	...
	}	private final Handler mTrafficHandler = new Handler(Looper.myLooper())
	{
		...
		private boolean shouldHide(long rxData, long txData, long timeDelta)
		{
			...
			switch(indicatorMode) {
				case MODE_SHOW_RXTX:
					lowSpeed = (speedRx < autoHideThreshold && speedTx < autoHideThreshold);
					break;
				case MODE_SHOW_RX:
	...
	protected void makeVisible(boolean trafficRelated)
	{
		...
		if(! mTrafficVisible || ! mViewVisible)
			return;
	...
	private void setIndicatorMode()
	{
		...
		mContext.getResources().getValue(mContext.getResources()
		                                         .getIdentifier("status_bar_icon_scale_factor", "dimen", mContext.getPackageName()), typedValue, true);
		float iconScaleFactor = typedValue.getFloat() * .65f;
		
		int Height = mContext.getResources().getDimensionPixelSize(mContext.getResources()
		                                                                   .getIdentifier("status_bar_battery_icon_height", "dimen", mContext.getPackageName()));
