...
public class NotificationIconContainerOverride
{
	...
	public static void calculateIconTranslations(XC_MethodHook.MethodHookParam param)
	{
		...
		int maxVisibleIcons = getBooleanField(thisObject, "mOnLockScreen") ? MAX_VISIBLE_ICONS_ON_LOCK : getBooleanField(thisObject, "mIsStaticLayout") ? MAX_STATIC_ICONS : childCount;
		...
		for(int i = 0; i < childCount; i++) {
			...
			boolean forceOverflow = mSpeedBumpIndex != - 1 && i >= mSpeedBumpIndex && getFloatField(iconState, "iconAppearAmount") > 0.0f || i >= maxVisibleIcons;
			boolean noOverflowAfter = i == childCount - 1;
			float drawingScale = getBooleanField(thisObject, "mOnLockScreen") && view.getClass() == StatusBarIconViewClass ? (float) callMethod(view, "getIconScaleIncreased") : 1f;
			setObjectField(iconState, "visibleState", getBooleanField(iconState, "hidden") ? STATE_HIDDEN : STATE_ICON);
		...
		if(firstOverflowIndex != - 1) {
			...
			for(int i = firstOverflowIndex; i < childCount; i++) {
				...
				if(getIntField(thisObject, "mNumDots") < MAX_DOTS) {
					...
					translationX += (getIntField(thisObject, "mNumDots") == MAX_DOTS ? MAX_DOTS * dotWidth : dotWidth) * getFloatField(iconState, "iconAppearAmount");
		...
		if(getObjectField(thisObject, "mIsolatedIcon") != null) {
			...
			if(iconState != null) {
				...
				setObjectField(iconState, "xTranslation", getFloatField(getObjectField(thisObject, "mIsolatedIconLocation"), "left") - mAbsolutePosition[0] - (1 - (float) callMethod(getObjectField(thisObject, "mIsolatedIcon"), "getIconScale")) * (float) callMethod(getObjectField(thisObject, "mIsolatedIcon"), "getWidth") / 2.0f);
				setObjectField(iconState, "visibleState", STATE_ICON);
			}
		}
	}
