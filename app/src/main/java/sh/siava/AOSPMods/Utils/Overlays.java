...
public class Overlays
{
	...
	{ //filling overlay list from resources, using a bit of reflection :D
		...
		for(Field filed : fileds) {
			try {
				...
				if(resources.getResourceName(resid)
				            .endsWith("OverlayEx") || resources.getResourceName(resid)
				                                               .endsWith("OverlayG"))
				{
					...
					Overlays.put(resources.getResourceName(resid)
					                      .replace("sh.siava.AOSPMods:array/", ""), new overlayGroup(resources.getResourceName(resid), members));
				}
