...
public class StringFormatter
{
	...
	private CharSequence networkStatOf(String variable)
	{
		...
		try {
			switch(variable) {
				...
				case "rx":
					textColor = RXColor;
					traffic = SystemUtils.NetworkStats()
					                     .getTodayDownloadBytes(variable.startsWith("c"));
					break;
				case "ctx":
				case "tx":
					textColor = TXColor;
					traffic = SystemUtils.NetworkStats()
					                     .getTodayUploadBytes(variable.startsWith("c"));
					break;
				case "call":
				case "all":
					traffic = SystemUtils.NetworkStats()
					                     .getTodayDownloadBytes(variable.startsWith("c")) + SystemUtils.NetworkStats()
					                                                                                   .getTodayUploadBytes(variable.startsWith("c"));
					break;
			}
	...
	{
		try {
			@SuppressLint("SimpleDateFormat") String result = new SimpleDateFormat(format).format(Calendar.getInstance()
			                                                                                              .getTime());
			hasDate = true;
			return result;
		} catch(Exception e) {
	...
	{
		try {
			String result = PersianDate.now()
			                           .format(DateTimeFormatter.ofPattern(format, Locale.forLanguageTag("fa")));
			hasDate = true;
			char[] bytes = result.toCharArray();
			NumericShaper.getShaper(NumericShaper.EASTERN_ARABIC)
			             .shape(bytes, 0, bytes.length); //Numbers to be shown in correct font
