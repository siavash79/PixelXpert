...
public class miscSettings extends XposedModPack
{
	...
	public void updatePrefs(String... Key)
	{
		if(XPrefs.Xprefs == null)
			return; //it won't be null. but anyway...
		
		//netstat settings
		try {
			Objects.requireNonNull(SystemUtils.NetworkStats())
				   .setStatus(XPrefs.Xprefs.getBoolean("NetworkStatsEnabled", false));
			Objects.requireNonNull(SystemUtils.NetworkStats())
				   .setSaveInterval(XPrefs.Xprefs.getInt("netstatSaveInterval", 5));
	...
	private void updateWifiCell()
	{
		...
		try {
			String currentTiles = Shell.cmd("settings get secure sysui_qs_tiles").exec().getOut()
									   .get(0);
			...
			com.topjohnwu.superuser.Shell.cmd("settings put global settings_provider_model " + providerModel + "; settings put secure sysui_qs_tiles \"" + currentTiles + "\"")
										 .exec();
		} catch(Exception ignored) {
		}
	}
	...
	private void updateSysUITuner()
	{
		try {
			...
			Shell.cmd("pm " + mode + " com.android.systemui/.tuner.TunerActivity").exec();
		} catch(Exception ignored) {
		}
	}
		...
		new Thread(() -> {
			try {
				if(customFontsEnabled && GSansOverrideEnabled) {
					Shell.cmd(String.format("cp %s/data/fontz/GSans/*.ttf %s/system/fonts/ && cp %s/data/productz/etc/fonts_customization.xml.NEW %s/system/product/etc/fonts_customization.xml", XPrefs.MagiskRoot, XPrefs.MagiskRoot, XPrefs.MagiskRoot, XPrefs.MagiskRoot))
						 .exec();
				} else if(customFontsEnabled) {
					Shell.cmd(String.format("rm -rf %s/system/fonts/*.ttf && cp %s/data/productz/etc/fonts_customization.xml.OLD %s/system/product/etc/fonts_customization.xml && cp -r %s/data/productz/fonts/* %s/system/product/fonts/", XPrefs.MagiskRoot, XPrefs.MagiskRoot, XPrefs.MagiskRoot, XPrefs.MagiskRoot, XPrefs.MagiskRoot))
						 .exec();
				} else {
					Shell.cmd(String.format("rm -rf %s/system/fonts/*.ttf && rm -f %s/system/product/etc/fonts_customization.xml && rm -rf %s/system/product/fonts/*", XPrefs.MagiskRoot, XPrefs.MagiskRoot, XPrefs.MagiskRoot))
						 .exec();
				}
			} catch(Exception ignored) {
			}
	...
	{
		return packageName.equals(AOSPMods.SYSTEM_UI_PACKAGE);
	}
