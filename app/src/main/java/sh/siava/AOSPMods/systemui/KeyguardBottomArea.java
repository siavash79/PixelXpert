...
public class KeyguardBottomArea extends XposedModPack
{
	...
	public KeyguardBottomArea(Context context)
	{
		super(context);
	}
	...
	public void handleLoadPackage(XC_LoadPackage.LoadPackageParam lpparam) throws Throwable
	{
		...
		findAndHookMethod(KeyguardbottomAreaViewClass, "onFinishInflate", new XC_MethodHook()
		{
			@Override
		...
		findAndHookMethod(KeyguardbottomAreaViewClass.getName() + "$" + "WalletCardRetriever", lpparam.classLoader, "onWalletCardsRetrieved", "android.service.quickaccesswallet.GetWalletCardsResponse", new XC_MethodHook()
		{
			@Override
		...
		//make sure system won't play with our button
		hookAllMethods(KeyguardbottomAreaViewClass, "updateControlsVisibility", new XC_MethodHook()
		{
			@Override
			protected void beforeHookedMethod(MethodHookParam param) throws Throwable
			{
				if(leftShortcut.length() == 0 || mControlsButton == null)
					return;
		...
		//make sure system won't play with our button
		findAndHookMethod(KeyguardbottomAreaViewClass, "updateWalletVisibility", new XC_MethodHook()
		{
			@Override
			protected void beforeHookedMethod(MethodHookParam param) throws Throwable
			{
				if(rightShortcut.length() == 0 || mWalletButton == null)
					return;
		...
		findAndHookMethod(KeyguardbottomAreaViewClass, "updateAffordanceColors", new XC_MethodHook()
		{
			@Override
			protected void afterHookedMethod(MethodHookParam param) throws Throwable
			{
				...
				int mTextColorPrimary = (int) callStaticMethod(UtilClass, "getColorAttrDefaultColor", mContext, mContext.getResources()
				                                                                                                        .getIdentifier("wallpaperTextColorAccent", "attr", mContext.getPackageName()));
		...
		findAndHookMethod(KeyguardbottomAreaViewClass.getName() + "$DefaultRightButton", lpparam.classLoader, "getIntent", new XC_MethodHook()
		{
			@Override
			protected void beforeHookedMethod(MethodHookParam param) throws Throwable
			{
	...
	private void convertShortcut(ImageView Button, String type, Object thisObject)
	{
		...
		switch(type) {
			case "camera":
				...
				drawable = ResourcesCompat.getDrawable(mContext.getResources(), mContext.getResources()
				                                                                        .getIdentifier("ic_camera_alt_24dp", "drawable", mContext.getPackageName()), mContext.getTheme());
				break;
			case "assistant":
				listener = v -> callMethod(thisObject, "launchVoiceAssist");
				drawable = ResourcesCompat.getDrawable(mContext.getResources(), mContext.getResources()
				                                                                        .getIdentifier("ic_mic_26dp", "drawable", mContext.getPackageName()), mContext.getTheme());
				break;
			case "torch":
				listener = v -> SystemUtils.ToggleFlash();
				drawable = ResourcesCompat.getDrawable(mContext.getResources(), mContext.getResources()
				                                                                        .getIdentifier("@android:drawable/ic_qs_flashlight", "drawable", mContext.getPackageName()), mContext.getTheme());
				break;
		}
		if(type.length() > 0) {
